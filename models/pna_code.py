...
class PNACode(nn.Module):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(PNACode, self).__init__()
        self.node_gnn = PNAGNNCode(hidden_dim=hidden_dim, aggregators=aggregators,
...
class PNAGNNCode(nn.Module):
    def __init__(self, hidden_dim, aggregators: List[str], scalers: List[str],
                 residual: bool = True, edge_features: bool = False, pairwise_distances: bool = False,
                 ...
                 **kwargs):
        super(PNAGNNCode, self).__init__()
        ...
        for _ in range(propagation_depth):
            self.mp_layers.append(
                PNALayerCode(in_dim=hidden_dim, out_dim=int(hidden_dim), in_dim_edges=in_dim_edges, aggregators=aggregators,
    ...
    def forward(self, graph: dgl.DGLGraph):
        graph.ndata['feat'] = self.node_encoder(graph.ndata['feat'], graph.ndata['depth'].view(-1,))
        if self.edge_features:
            raise NotImplementedError
...
class PNALayerCode(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, in_dim_edges: int, aggregators: List[str], scalers: List[str],
        ...
        super(PNALayerCode, self).__init__()

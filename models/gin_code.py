...
from commons.ogb_code_utils import ASTNodeEncoder, NUM_NODETYPES, NUM_NODEATTRIBUTES


class OGBGNNCode(nn.Module):
    ...
    def __init__(self, target_dim = 1, num_layers = 5, hidden_dim = 300, gnn_type = 'gin',
        ...
        '''
        super(OGBGNNCode, self).__init__()
        ...
        if virtual_node:
            self.node_gnn = GNNCode_node_Virtualnode(num_layers, hidden_dim, JK = JK,
                                                 dropout = dropout,
                                                 residual = residual,
                                                 gnn_type = gnn_type, batch_norm_momentum=batch_norm_momentum)
        else:
            self.node_gnn = GNNCode_node(num_layers, hidden_dim, JK = JK, dropout = dropout,
...
class GCNConvCode(nn.Module):
    def __init__(self, hidden_dim):
        '''
            hidden_dim (int): node embedding dimensionality
        '''

        super(GCNConvCode, self).__init__()
...
class GNNCode_node(nn.Module):
    ...
    def __init__(self, num_layers, hidden_dim, dropout = 0.5, JK = "last", residual = False, gnn_type = 'gin', batch_norm_momentum=0.1):
        ...
        super(GNNCode_node, self).__init__()
        self.num_layers = num_layers
        self.dropout = dropout
        self.JK = JK
        ...
        self.node_encoder = ASTNodeEncoder(emb_dim=hidden_dim, num_nodetypes=NUM_NODETYPES,
        ...
        for layer in range(num_layers):
            if gnn_type == 'gin':
                self.convs.append(GINConvCode(hidden_dim, batch_norm_momentum))
            elif gnn_type == 'gcn':
                self.convs.append(GCNConvCode(hidden_dim))
            else:
    ...
    def forward(self, g, x):
        ...
        h_list = [self.node_encoder(x, g.ndata['depth'].view(-1,))]
        for layer in range(self.num_layers):

            h = self.convs[layer](g, h_list[layer])
...
class GNNCode_node_Virtualnode(nn.Module):
    """
    Output:
        node representations
    """
    def __init__(self, num_layers, hidden_dim, dropout = 0.5, JK = "last", residual = False, gnn_type = 'gin', batch_norm_momentum=0.1):
        ...
        super(GNNCode_node_Virtualnode, self).__init__()
        self.num_layers = num_layers
        self.dropout = dropout
        self.JK = JK
        ### add residual connection or not
        ...
        for layer in range(num_layers):
            if gnn_type == 'gin':
                self.convs.append(GINConvCode(hidden_dim))
            elif gnn_type == 'gcn':
                self.convs.append(GCNConvCode(hidden_dim))
            else:
                ValueError('Undefined GNN type called {}'.format(gnn_type))
